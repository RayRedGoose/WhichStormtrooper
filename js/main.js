...
function chooseBodyClickAction(event) {
  ...
  if (event.target.classList.contains('button--new-game')) { renewPlayerAndGame(); }
  if (event.target.classList.contains('button--rematch')) { rematchGame(event.target); }
}

function launchGame(target) {
  var start = Date.now();
  deck.startTime = start;
...
function createFirstPage() {
  ...
  section.classList.add('player-input-form');
  section.innerHTML = `
  <label for="player-input">
    <input id="player-input" type="text" placeholder="Your name here">player name
  </label>
  <button class="main-page button--play-game" type="button" name="button">play game</button>`;
...
function removeWinnerPage() {
  document.querySelector('.winner-information').remove();
}
...
function showRecentGames() {
  if (games.length != 0 && games.length < 4) {
    for (var i = 0; i < games.length; i++) {
      var parent = document.querySelector('.win-game-list');
      var article = document.createElement('article');
      article.innerHTML = `
        <p class="aside-text">round <span class="round-number">${games[i].round}</span></p>
        <p class="aside-text"><span class="game-time">${games[i].time.minutes}</span> minutes <span class="game-time">${games[i].time.seconds}</span> seconds</p>`;
      parent.appendChild(article);
    }
  }
  if (games.length > 3) {
    for (var i = games.length - 1; (games.length - 4 < i || games.length - 0 < i); i--) {
      var parent = document.querySelector('.win-game-list');
      var article = document.createElement('article');
      article.innerHTML = `
        <p class="aside-text">round <span class="round-number">${games[i].round}</span></p>
